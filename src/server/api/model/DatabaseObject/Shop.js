export default function (dataObject, permissions) {
    var here = {
        ...
        AddProduct: (idAccount, name, description, price) => {
            return new Promise((resolve, reject) => {
                permissions.FilterPermission(idAccount, "P_ADD_SHOP").then(() => {
                    dataObject.Produit.findOrCreate({
                        where: {
                            Nom: name,
        ...
        GetCategoriesID: (idProduct) => {
            return dataObject.Regroupe.findAll({
                where: {
                    ID: idProduct
                }
            });
        ...
        GetCategorieFromID: (idCategorie) => {
            return dataObject.Categorie.findOne({
                where: {
                    ID: idCategorie
                }
            });
        ...
        GetProductFromID: (idProduct) => {
            return dataObject.Produit.findOne({
                where: {
                    ID: idProduct
                }
            });
        ...
        AddItemToPurchaseList: (idAccount, idProduct, quantity) => {
            return new Promise((resolve, reject) => {
                permissions.FilterPermission(idAccount, "P_PURCHASE_SHOP").then(() => {
                    dataObject.Achats.findOrCreate({
                        where: {
                            Realise: false,
                            ID_Compte: idAccount
                        }
                    }).then(r => {
                        dataObject.Panier.findOrCreate({
                            where: {
                            ...
                            if (!s[1]) {
                                dataObject.Panier.update({
                                    Quantite: r[0].Quantite + quantity
                                }, {
                                    where: {
                                        ID: idAccount,
                                        ID_Produit: idProduct
        ...
        GetPurchaseListOfUser: (idAccount) => {
            return new Promise((resolve, reject) => {
                dataObject.Achats.findOne({
                    where: {
                        ID_Compte: idAccount,
                        Realise: false
                    }
                }).then(r => {
                    dataObject.Panier.findAll({
                        where: {
                            ID: r.ID
                        }
                    }).then(s => {
        ...
        CommitPurchase: (idAccount) => {
            return dataObject.Achats.update({
                Realise: true
            }, {
                where: {
                    ID_Compte: idAccount
        ...
        GetHistoryOfPurchase: (idAccount) => {
            return dataObject.Achats.findAll({
                where: {
                    ID_Compte: idAccount,
                    Realise: true
                }
        ...
        CreateCategorie: (idAccount, categorie) => {
            return new Promise((resolve, reject) => {
                permissions.FilterPermission(idAccount, "P_SET_CATEGORIE_SHOP").then(() => {
                    dataObject.Categorie.findOrCreate({
                        where: {
                            Nom: categorie
                        }
                    }).then(r => {
        ...
        AddItemToCategorie: (idAccount, idCategorie, idProduct) => {
            return new Promise((resolve, reject) => {
                permissions.FilterPermission(idAccount, "P_SET_CATEGORIE_SHOP").then(() => {
                    dataObject.Regroupe.findOrCreate({
                        where: {
                            ID: idProduct,
                            ID_Categorie: idCategorie
                        }
